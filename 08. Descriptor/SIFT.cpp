...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold) {
			double minDist = euclidDistance(desc1,descriptors2.row(nn));
			double distance2 = 1e6;
			for (int i = 0; i < descriptors2.rows; i++) {
				Mat v = descriptors2.row(i);		// each row of descriptor
				double distance = euclidDistance(desc1, v);
				if (distance > minDist && distance < distance2) {
					distance2 = distance;
				}
			}
			if (minDist / distance2 >= RATIO_THR)
				continue;
		}
