...
int main() {
	...
	//forward warping
	/*for (int i = 0; i < I2_row; i++) {
		for (int j = 0; j < I2_col; j++) {
			float x = A21.at<float>(0) * i + A21.at<float>(1) * j + A21.at<float>(2)+diff_x;
			float y = A21.at<float>(3) * i + A21.at<float>(4) * j + A21.at<float>(5)+diff_y;

			I_f.at<Vec3f>(round(x), round(y)) = I2.at<Vec3f>(i, j);
		}
	}*/
	...
	for (int i = -diff_x_; i < I_f.rows - diff_x_; i++) {
		for (int j = -diff_y_; j < I_f.cols - diff_y_; j++) {
			...
			{
				I_f.at<Vec3f>(i+diff_x_, j+diff_y_) = lambda*(mu*I2.at<Vec3f>(x2, y2)+ (1 - mu)*I2.at<Vec3f>(x1, y2))
					+ (1 - lambda)*(mu*I2.at<Vec3f>(x2, y1) + (1 - mu)*I2.at<Vec3f>(x1, y1));
			}
		}
	}
...
Mat cal_affine(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points) {
	...
	for (int i = 0; i < number_of_points; i++) {
		M.at<float>(2*i, 0) = ptl_x[i]; M.at<float>(2*i, 1) = ptl_y[i]; M.at<float>(2*i, 2) = 1;
		M.at<float>(2 * i, 3) = M.at<float>(2 * i, 4) = M.at<float>(2 * i, 5) = 0;
		M.at<float>(2 * i + 1, 0) = M.at<float>(2 * i + 1, 1) = M.at<float>(2 * i + 1, 2) = 0;
		M.at<float>(2 * i+1, 3) = ptl_x[i]; M.at<float>(2 * i+1, 4) = ptl_y[i]; M.at<float>(2 * i+1, 5) = 1;
		b.at<float>(2*i) = ptr_x[i]; b.at<float>(2*i + 1) = ptr_y[i];
	}
